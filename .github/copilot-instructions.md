...
- **Target**: Prioritize caching for tables that are **read frequently but updated rarely** (e.g., settings, permissions, types, categories).
- **Avoid**: Do not cache highly volatile data or real-time metrics unless necessary for performance.

## Implementation

- **Wrapper**: Use `Cache::remember()` or `Cache::rememberForever()` to handle retrieval and storage in one call.
- **Keys**: Use predictable, scoped keys (e.g., `user:1:permissions`, `settings:global`).
- **TTL**: Define specific Time-To-Live durations using `now()->add*()` methods.

## Invalidation

- **Events**: Invalidate cache keys on Model `saved`, `updated`, or `deleted` events.
- **Observers**: Use Model Observers for complex invalidation logic.
